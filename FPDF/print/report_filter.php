...
function ItemWhereIn($sql){
    ...
    try
    {
        $st = $pdo->prepare($query);
        $st->execute();
        $row = $st->fetchAll(PDO::FETCH_COLUMN);
    }catch(PDOException $ex){
        echo $ex->getMessage();
        exit();
    }
...
$string=$_GET['select_val'];
$array_item=array_map('intval', explode(',', $string));
$array_item = implode("','",$array_item);
$sql_item = "SELECT expense_id FROM item WHERE id IN ('".$array_item."')";
$itemWhereIn = ItemWhereIn($sql_item);

$array_expense=array_map('intval', $itemWhereIn);
$array_expense = implode("','",$array_expense);
$sql_expense = "SELECT * FROM expense WHERE id IN ('".$array_expense."') and division ='".$_GET['division']."'";
$expenseWhereIn = ExpenseWhereIn($sql_expense);

$grand_total = 0;
foreach($expenseWhereIn as $expense){
    $count_first = 0;
    $count_second = 0;
    $alphabet = range('A', 'Z');
    ...
    if(isset($expense_code)){
        foreach($expense_code as $display_first => $first){
            foreach($first as $display_second){
                ...
                if($_GET['status'] == 'inactivate'){
                    ...
                    $items = queryItem("SELECT item.*,mode_procurement.description as mode_procurement_description FROM ITEM left join mode_procurement on mode_procurement.id = item.mode_procurement where item.tranche = '$tranche' and item.expense_id = '$expense->id' and (item.status = 'approve' or item.status = 'pending') and item.id IN ('".$array_item."') ");
                }
        ...
        if($_GET['status'] == 'inactivate') {
            ...
            $items = queryItem("SELECT item.*,mode_procurement.description as mode_procurement_description FROM ITEM left join mode_procurement on mode_procurement.id = item.mode_procurement where item.expense_id = '$expense->id' and (item.status = 'approve' or item.status = 'pending') and item.id IN ('".$array_item."') ");
        }
        foreach($items as $item){
            $pdf->SetFont('Arial','',7);
            $pdf->displayItem($item);
