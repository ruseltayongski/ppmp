...
function querySection(){
    $pdo = conn_program();
    $query = "SELECT * FROM SECTION WHERE DIVISION = 4 ORDER BY ID ASC";
...
foreach($sections as $section) {
        foreach($expenses as $expense) {
            ...
            if(isset($expense_code)){
                foreach($expense_code as $display_first => $first){
                    foreach($first as $display_second){
                        ...
                        foreach($programs as $program) {
                            if ($program->section_id == $section->id)
                ...
                $items = queryItem("call normal_tranche_reg_pr('$expense->id','$ppmp_status')");
